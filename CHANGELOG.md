...
- Steam-styled **toast notifications** for setting changes, save confirmation, and clearing the blocked-games list (title, message, status kind, auto-dismiss with hover pause)
- General settings: toggle toasts on/off and choose corner position (default top-right)
...
- Guest search suggestions show release date next to price/platforms (priority enrich for blocked/sparse rows; `coming soon` when Steam has no date yet)

## [1.16.9] - 2026-07-13

### Fixed

- Sparse / region-blocked guest search rows now backfill price, platforms, release date, metascore, and reviews via anonymous `appdetails` + `appreviews` (same guest stack as store pages)
